...
public class GameManagerPuzz : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Inicializar sistema de tiempo y sonido
        tiempoRestante = tiempoLimite;
        juegoActivo = true;
        puzzleCompletado = false;
        shuffling = true; // Iniciar como shuffling para evitar detección prematura de completado
    ...
    void Update()
    {
        ...
        if (tiempoRestante <= 0 && !puzzleCompletado)
        {
            GameOver();
            return;
        }

        // Check for completion (solo después del shuffle inicial)
        if (!shuffling && !puzzleCompletado && CheckCompletion())
        {
